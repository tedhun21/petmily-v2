...
// 1. 과거 메시지 로딩 및 관리
// 2. 읽음 처리 서버 전송 로직 (수동 디바운스 및 플러시 포함)
// 3. 컴포넌트 언마운트시 읽음 처리 강제 전송
// 4. 메시지 가시성 감지 및 읽음 처리 트리거
// 5. 실시간 새 메시지 수신 관리
// 6. 상대방의 읽음 처리 업데이트 수신 및 UI 반영
// 7. 메시지 관련 데이터 및 콜백 제공
export default function MessageProvider({ children }: any) {
  ...
  // 2. 읽음 처리 서버 전송 로직 (useDebounce 훅 사용)
  ...
  // 3. 컴포넌트 언마운트/채팅방 변경 시 읽음 처리 강제 전송 (디바운스 대기중인 값 즉시 실행)
  useEffect(() => {
    return () => {
  ...
  const onMessageVisible = useCallback(
    (message: Message) => {
      ...
      const isAfter = isAfterMessage(message, current);
  ...
  useEffect(() => {
    ...
    const handleReadUpdateFromServer = (receivedData: { lastReadMessage: Message; readBy: number }) => {
      ...
      setChatRoom((prevChatRoom: ChatRoom | null) => {
        ...
        const updatedOthers = prevChatRoom.chatMembers.others.map((member: ChatMember) => {
          if (member.user.id === readBy) {
            const currentLastRead = member.lastReadMessage;

            if (!currentLastRead || isAfterMessage(lastReadMessage, currentLastRead)) {
              return {
                ...member,
                lastReadMessage,
              };
  ...
  return (
    <MessageContext.Provider
      value={{ messages, newMessages, allMessages, setSize, isLoading, isValidating, isEnd, onMessageVisible }}
    >
      {children}
    </MessageContext.Provider>
  );
